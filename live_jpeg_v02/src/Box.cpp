#include "Box.h"
	...
	//sj_jumbf_super_box_ptr sj_create_jumbf_xml_box(unsigned char* xml, char* label);
	//sj_jumbf_xml_box_ptr create_xml_box(char* xml);
	...
	sj_jumbf_super_box_ptr sj_create_jumbf_jpeg360_box(unsigned char* xml, unsigned int xml_size, char* label)
	{
		...
		sj_jumbf_xml_box_ptr xml_box = create_xml_box((char*)xml, xml_size);


		//signature 추후에 추가 desc_box 에 포함되는 content box들의 stream을 sha256을 통해 hash를 구함
		//id는 사용자 지정 이곳에서는 추가하지 않음
		sj_jumbf_desc_box_ptr desc_box = create_desc_box(TYPE_JPEG360, label, 0, NULL);
		desc_box->next_box = (sj_jumbf_box_ptr)xml_box;

		jumbf_box->length = xml_box->length + desc_box->length + 8;
		jumbf_box->box_type = BOX_TYPE_JUMB;
		jumbf_box->next_box = (sj_jumbf_box_ptr)desc_box;
		return jumbf_box;
	}

	sj_jumbf_super_box_ptr sj_create_jumbf_xml_box(unsigned char* xml, unsigned int xml_size, char* label)
	{
		sj_jumbf_super_box_ptr jumbf_box = (sj_jumbf_super_box_ptr)malloc(sizeof(sj_jumbf_super_box));
		memset(jumbf_box, 0, sizeof(sj_jumbf_super_box));
		sj_jumbf_xml_box_ptr xml_box = create_xml_box((char*)xml, xml_size);
		...
		//signature 추후에 추가 desc_box 에 포함되는 content box들의 stream을 sha256을 통해 hash를 구함
		//id는 사용자 지정 이곳에서는 추가하지 않음
		sj_jumbf_desc_box_ptr desc_box = create_desc_box(TYPE_XML, label, 0, NULL);
		desc_box->next_box = (sj_jumbf_box_ptr)xml_box;
	...
	sj_jumbf_xml_box_ptr create_xml_box(char* xml, unsigned int xml_size)
	{
		sj_jumbf_xml_box_ptr xml_box = (sj_jumbf_xml_box_ptr)malloc(sizeof(sj_jumbf_xml_box));
		memset(xml_box, 0, sizeof(sj_jumbf_xml_box));

		unsigned int length = xml_size + 8; //length 4 byte, box type 4 bytpe, 문자열 끝 \0 1byte
		unsigned int box_type = BOX_TYPE_XML;
		xml_box->xml = (char*)calloc(length - 8, sizeof(char));
		xml_box->length = length;
		xml_box->box_type = box_type;

		memcpy(xml_box->xml, xml, length - 8);
		xml_box->next_box = NULL;
		return xml_box;
	}
	...
	void destroy_jumbf_box(sj_jumbf_super_box_ptr box)
	{
		...
		while (next_box != NULL)
		{
			...
			else if (next_box->box_type == BOX_TYPE_XML)
			{
				sj_jumbf_xml_box_ptr xml_box = (sj_jumbf_xml_box_ptr)(next_box);

				if (xml_box->xml != NULL)
					free(xml_box->xml);

				next_box = xml_box->next_box;
				free(xml_box);
			}
		...
		*dst_size = box_ptr->length;
		...
		while (next_box != NULL)
		{
			...
			else if (next_box->box_type == BOX_TYPE_XML)
			{
				sj_jumbf_xml_box_ptr xml_box = (sj_jumbf_xml_box_ptr)(next_box);
				//length
				emit_4byte(&dst_buf_position, xml_box->length);

				//box type
				emit_4byte(&dst_buf_position, xml_box->box_type);

				//xml_data
				for (int i = 0; i < (xml_box->length - 8); i++)
				{
					emit_byte(&dst_buf_position, xml_box->xml[i]);
				}
				next_box = xml_box->next_box;
			}
			else if (next_box->box_type == BOX_TYPE_CODESTREAM) {
