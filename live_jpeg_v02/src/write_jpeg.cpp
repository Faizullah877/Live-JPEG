#include "write_jpeg.h"
	...
	void create_jumd_buf(Byte** jumd_buf, uint* jumd_buf_size, sj_jumbf_super_box* super_box)
	{
		...
		while (next_box != NULL)
		{
			...
			else if (next_box->box_type == BOX_TYPE_FAIZ) {
				sj_jumbf_sos_lengths_box_ptr sosL_box = (sj_jumbf_sos_lengths_box_ptr)(next_box);
				next_box = sosL_box->next;
			}
		}

		//destroy_jumbf_box(super_box);
		//free(super_box);
	}
	...
	bool jpeg_write_sos_jumbf(unsigned char* input_jpeg, unsigned int input_jpeg_size, sj_jumbf_super_box* sos_super_box, unsigned char** dst_buf, unsigned int* dst_size)
	{
		...
		uint amount_per_marker = 65000;  // total capacity is 65535;
		...
		while (next_box != NULL)
		{
			...
			else if (next_box->box_type == BOX_TYPE_XML)
			{
				...
				sj_jumbf_sos_lengths_box_ptr sosL_box = (sj_jumbf_sos_lengths_box_ptr)(next_box);
				codestream = sosL_box->codestream;
				codestream_size = sosL_box->length - 8;
				next_box = sosL_box->next;
			}
		}


		uint remaining_data = codestream_size;

		uint all_app11_size = no_of_app11_markers_req * (2 + 2 + 2 + 2 + 4 + 4 + 4 + jumd_buf_size + 8) + codestream_size; // 2 app11, 2 Le, 2 CI, 2 En, 4 Z, 4 Lbox, 4 Tbox, 8 jumbcodestream box lenght and type
		...
		do {
			...
			if (val1 == 0xFF)
			{
				...
				case M_DQT:
				{
					...
					buf -= 2;
					buf_size -= 2;
					*dst_size = input_jpeg_size + all_app11_size;

					*dst_buf = new unsigned char[*dst_size];
					unsigned char* dst_buf_position = *dst_buf;
					memcpy(dst_buf_position, input_jpeg, buf_size);// copies data till SOS from source
					...
					uint code_strm_box_length = 0;
					for (uint i = 0; i < no_of_app11_markers_req; ++i) {
						...
						emit_4byte(&dst_buf_position, sos_super_box->length); // Lbox same for every marker
						...
						emit_4byte(&dst_buf_position, BOX_TYPE_FAIZ);
					...
					memcpy(dst_buf_position, buf, input_jpeg_size - buf_size);
			...
			if (buf > input_jpeg + input_jpeg_size)
			{
	...
	bool jpeg_write_jumbf(unsigned char* primary_frame, unsigned int primary_frame_size, sj_jumbf_super_box* super_box, unsigned char** dst_buf, unsigned int* dst_size)
	{
		...
		uint no_of_app11_markers_req = super_box->length / amount_per_marker; // one extra is needed for remainder
		no_of_app11_markers_req++;
